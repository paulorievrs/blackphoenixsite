        ...
        <div class="my-3 p-3 bg-white rounded shadow-sm">
            ...
            @foreach($jogos as $jogo)
                ...
                <div class="d-flex text-muted pt-3">
                    ...
                    <img class="bd-placeholder-img flex-shrink-0 me-2 rounded" src="{{ $time->teamLogo === null || strlen($time->teamLogo) === 0 ? "/img/logo.png" : $time->teamLogo }}" alt="" width="40"  height="40">
                    <p class="pb-3 mb-0 small lh-sm border-bottom w-100">
                        <strong class="d-block text-dark pb-3">Black Phoenix <span class="{{ $win ? "text-success" : "text-danger" }}">({{ $jogo->resultadoBlackPhoenix === null ? 0 : $jogo->resultadoBlackPhoenix }})</span> vs {{ $time->teamName }} <span class="{{ $win ? "text-danger" : "text-success" }}">({{ $jogo->resultadoDoTime === null ? 0 : $jogo->resultadoDoTime }})</span></strong>
                        ...
                        <br><br><strong class="text-dark">Campeonato: </strong> {{ $campeonato->name }}
                    </p>
